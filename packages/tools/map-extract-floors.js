...
var parseBoundingBoxCSV = (content) => {
  content.split(/\n/).filter((s) => s !== "").filter((s) => s.match(/^[A-Z]/)).map((s) => {
...
function buildAddress(n, parents) {
  ...
  const prefix = parents.filter(getLabel).map(getLabel).join("-").replace(/-Content-/, "-").replace(/^/, "A");
...
var snakeToCamel = (k) => {
  if (k.match(/^.*-.*$/)) {
    return k.split(/-/).map((s, i) => i === 0 ? s : capitalize(s)).join("");
  }
  return k;
};
...
var HIDDEN_GROUPS_RE = /(\(Background\)|\(Names\)|\(Unresolved Names\)|Shops|Facilities)/;
...
var fixupAttributes = (tree) => {
  return tree === void 0 ? tree : map(tree, (x2) => {
    if (is(x2, "element")) {
      return {
        ...x2,
        attributes: pipe(
...
var saveAllFloorLayerNames = (ast) => {
  visitParents(ast, (n) => {
    if (is2(n, "element") && n.name === "g") {
      ...
      if (n.attributes["inkscape:groupmode"] === "layer" && // exclude e.g. (Assets)
      label?.match(/[^(]/)) {
  ...
  const replacer = (k, v) => {
    if (v instanceof Map) {
      ...
      for (const [mk, mv] of v.entries()) {
        if (typeof mk === "string" && typeof mv === "object" && "x" in mv && typeof mv.x === "number" && "y" in mv && typeof mv.y === "number" && "r" in mv && typeof mv.r === "number") {
...
var renderAddressesTs = (layers) => {
  ...
  const doc = Doc3.vsep([
    ...
    Doc3.vsep(
      names.map(
        (n) => Doc3.text(`import addresses${n}Json from './addresses_${n}.json'`)
      )
    ),
    Doc3.text(``),
    Doc3.text(`export type Address =`),
    Doc3.vsep(names.map((x2) => Doc3.text(`  | keyof typeof addresses${x2}Json`))),
    Doc3.text(``),
...
var saveViewBox = (ast) => {
  ...
  visitParents7(ast, (n, parents) => {
    if (parents.length === 1 && is8(parents[0], "root") && is8(n, "element") && n.name === "svg") {
